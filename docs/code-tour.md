...
```
Browser (Svelte 5)  ──►  SvelteKit Server  ──►  SubbitMan + Subbit.xyz (L2 service layer)
                                           ──►  Orcfax Validator (data source)
                                           ──►  Cardano (L1)
```

The **browser** handles wallet connection, key management, IOU signing, and the data display. The **SvelteKit server** is a middleware proxy: it validates credentials with SubbitMan, fetches data from the validator, charges the cost, and returns results. It never holds private keys. **[SubbitMan](https://github.com/kompact-io/subbit-man-js)** is the L2 accounting engine that tracks how much each consumer has spent and authorized. **[Subbit.xyz](https://github.com/kompact-io/subbit-xyz)** is the L2 Aiken smart contract on Cardano that enforces the payment channel rules: escrow, settlement, deadlines. The **Orcfax Validator Node** is the upstream data source — it knows nothing about the other layers. And Cardano, of course, is the L1. The Subbit protocol handles most of the I/O with the Cardano L1 via [Blockfrost](https://blockfrost.io/).
...
1. **Connect wallet** — `Wallet.connect()` calls `BrowserWallet.enable()` via [Mesh SDK](https://meshjs.dev/). The connection object, network ID, and wallet name are stored in `$state` fields. localStorage remembers the wallet name for auto-reconnect.

2. **Generate keys** — `AuthKey.create()` generates a random Ed25519 private key and derives the public key. Both are stored as hex strings in `$state`. Once the channel opens, the key is cached in IndexedDB and offered as a downloadable JSON keyfile.
...
4. **Fetch or publish a price** — `ODAPI.updateFeedPrice("ADA-USD")` calculates the next IOU amount (`Channel.getNextIouAmount`), signs an IOU credential, calls the `getPrices` remote function, stores the price in IndexedDB, and syncs the channel to update accounting.

5. **Close & withdraw** — `Channel.close()` builds and submits a Close transaction, starting the settlement window. The provider settles (claiming authorized funds) or the deadline expires. Then `Channel.withdraw()` is enabled for the user to dispatch to reclaim remaining funds.
...
**keyCache** ([`src/lib/subbit/keyCache.ts`](https://github.com/orcfax/on-demand/blob/main/src/lib/subbit/keyCache.ts)) is an opt-in IndexedDB store for Ed25519 private keys. When the user checks "Remember this key on this device" during channel setup (or via the channel settings menu), the private key is cached under the channel's keytag. On return visits, the restore flow skips the keyfile upload entirely and auto-unlocks. The cache is deliberately unencrypted — it mirrors the security posture of the downloadable JSON keyfile.
...
Remote functions serve the portal UI, but there's a full parallel REST API at `src/routes/api/` for headless usage of the application — scripts, SDKs, anything that speaks HTTP.
...
## Code Tour: The Core Subbit Protocol
...
An unusual aspect of this system: the Terms of Service are not a legal page bolted on at the end — they're a first-class off-chain data structure that drives pricing and protocol behavior.
...
Decisions and reasoning behind the choices made for the Subbit protocol (cumulative IOUs, mono-asset channels, datum-stored constants) are out-of-scope for this doc, but are documented in the Subbit project's [ADRs](https://github.com/orcfax/on-demand/tree/main/services/subbit-xyz/docs/adrs); below we focus on the application-level choices specific to this portal and it's interaction with Subbit.
...
Remote functions are experimental, but pretty stable at this point. We chose them because:
...
Two reasons. First, **security isolation**. The IOU key is a disposable hot key. Its only power is authorizing payments to the provider for a specific channel — if compromised, the worst case is the attacker lets the provider claim more of the consumer's already-escrowed funds. The wallet key controls the channel closure and fund withdrawal. Separating these keys means a compromised IOU key can't drain the wallet, add more funds from the wallet to the escrow, or close the channel.
...
- **Mesh SDK** on the client — because it provides a great CIP-30 wallet integration. `BrowserWallet.enable()`, `signTx()`, `submitTx()` — the wallet interaction surface is Mesh's strength.

- **Lucid Evolution** on the server (in SubbitMan) — because it provides a great transaction building API for script interactions. Building transactions that spend from validators, attach redeemers, and handle Plutus datums is Lucid's strength.

We tried using one or the other for everything but ran into various issues in doing so. In the interest of development speed, we opted for this split: the client never needs Lucid (saving ~500KB of bundle size) and the server never needs Mesh (avoiding CIP-30 browser dependencies).
